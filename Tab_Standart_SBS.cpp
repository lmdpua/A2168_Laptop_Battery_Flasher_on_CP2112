...
void MainWindow::setTableStandardSBS (QTableView *tablePlace, QStandardItemModel *&tableName, quint8 start, quint8 end)
{
...
//    tablePlace->horizontalHeader()->setDefaultSectionSize(23); //Ширина строки по умолчанию
    ...
    tablePlace->setColumnWidth(1, 145);          //Ширина первого(нулевого) столбца
    tablePlace->setColumnWidth(2, 100);          //Ширина первого(нулевого) столбца
    tablePlace->setColumnWidth(3, 50);           //Ширина первого(нулевого) столбца
    tablePlace->setColumnWidth(4, 50);           //Ширина первого(нулевого) столбца
    ...
    for (quint8 i=start; i<end; i++)
    {
        QString str = QString("%1")                 //Строка, в которой будет один аргумент
                            .arg(Standard_Commands[i].addr, //А вот и аргумент
                                 2,                 //Четыре символа
                                 16,                //HEX
        ...
        //Небольшой костыль, чтобы увеличить высоту двух ячеек
        if(Standard_Commands[i].addr==0x23){
            tablePlace->setRowHeight(i-start,60);
        }
        if(Standard_Commands[i].addr==0x2f){
            tablePlace->setRowHeight(i-start,60);
        }
    }
    tableName->setVerticalHeaderLabels(RowLabels); //Проименуем заголовки строк значениями из списка
    ...
    for(quint8 i=0; i<row; i++)
    {
        ...
        switch (Standard_Commands[i+start].access) {
        case R:
        ...
        tableName->setData(indexName, Standard_Commands[i+start].name); //Отобразим параметр в ячейке index

        indexUnit = tableName->index(i,3); //Найдем индекс нужной ячейки
        tableName->setData(indexUnit, Standard_Commands[i+start].unit); //Отобразим параметр в ячейке index
...
void MainWindow::setDataTableStandardSBS (QStandardItemModel *&tableName, quint8 start, quint8 end, CP2112 *dev, SMBus *batt)
{
    ...
    for(quint8 i=start; i<end; i++){
        ...
        switch ((quint8)Standard_Commands[i].format){

        case HEX2:
            batt->readWord(dev, Standard_Commands[i].addr, &data);
        ...
        case UINT2:
            batt->readWord(dev, Standard_Commands[i].addr, &data);
            if(Standard_Commands[i].unit == "d-m-y"){//Если дата
                QDate date; //Объявляем переменную date
                quint16 day = 0b00011111 & data;
                quint16 month = data >> 5 & 0b00001111;
                quint16 year = 1980 + (data >> 9 & 0b01111111);
        ...
        case STR20:
            if (Standard_Commands[i].addr == 0x23){
                batt->readBlock(dev, Standard_Commands[i].addr,&array,&len);
                str.clear();
...
void MainWindow::on_button_StandartSBS_ReadOnce_clicked()
{
    quint8 index = ui->tabWidget->indexOf(ui->tab_Standart_SBS);
    ...
    setDataTableStandardSBS(tableStandartSBS_Left, 0, 21, &device, &battery);
    setDataTableStandardSBS(tableStandartSBS_Right, 21, 38, &device, &battery);

    device.close();
    device.setTransfer(false);//снимем флаг, что устройство в режиме передачи
}

void MainWindow::on_button_StandartSBS_ReadAllTheTime_clicked()
{
    timerStandartSBSReading->start(2000); // Запустим таймер на 2 секунды
}
